...
#define MY_PORT  50000 // w26server
#define M1_PORT  50001 // mirror1
#define M2_PORT  50002 // mirror2
#define MX_DIRS 2000
...
// called in a forked child when this connection belongs to a mirror
void send_to_mirror(int client_sd, int mirror_port) {
    int mirror_fd; // socket to connect to mirror server
    struct sockaddr_in maddr; // addr of mirror server
    char buf[4096];
    fd_set rset; // for select to wait on both client_sd and mirror_fd
    int maxfd, n;

    mirror_fd = socket(AF_INET, SOCK_STREAM, 0); // connect to the mirror server running on the same machine
    if (mirror_fd < 0) {
        write(client_sd, "Mirror unavailable", 18);
        close(client_sd);
        return;
    }
    ...
    maddr.sin_port = htons((uint16_t)mirror_port);
    inet_pton(AF_INET, "127.0.0.1", &maddr.sin_addr);

    if (connect(mirror_fd, (struct sockaddr *)&maddr, sizeof(maddr)) < 0) {
        // if mirror is not running inform client and disconnect
        write(client_sd, "Mirror not available. Is it running?", 36);
        close(mirror_fd);
        close(client_sd);
        return;
    }

    // relay loop to forward bytes in both directions until one side closes
    maxfd = (client_sd > mirror_fd) ? client_sd : mirror_fd;
    while (1) { // wait for data from either client or mirror and forward to the other
        FD_ZERO(&rset);
        FD_SET(client_sd, &rset);
        FD_SET(mirror_fd, &rset);

        if (select(maxfd + 1, &rset, NULL, NULL, NULL) < 0) // wait for data on either socket
            break;

        // forward to mirror the client request
        if (FD_ISSET(client_sd, &rset)) {
            n = read(client_sd, buf, sizeof(buf));
            if (n <= 0) break; // client disconnected
            write(mirror_fd, buf, n);
        }

        // forward to client the response from mirror
        if (FD_ISSET(mirror_fd, &rset)) {
            n = read(mirror_fd, buf, sizeof(buf));
            if (n <= 0) break; // mirror disconnected
            write(client_sd, buf, n);
        }
    }
...
int main(void) {
    ...
    int conn_count = 0; // total clients accepted, parent increments this before fork
    struct sockaddr_in servAdd;

    signal(SIGCHLD, SIG_IGN); // prevent zombie processes by ignoring SIGCHLD
    ...
    setsockopt(lis_sd, SOL_SOCKET, SO_REUSEADDR, &opt, sizeof(opt)); // allow quick reuse of the port after server restarts

    memset(&servAdd, 0, sizeof(servAdd));
    servAdd.sin_family = AF_INET;
    servAdd.sin_addr.s_addr = htonl(INADDR_ANY);
    servAdd.sin_port = htons((uint16_t)MY_PORT); // use hardcoded port
    ...
    listen(lis_sd, 10); // 10 backlog for pending
    printf("w26server started. Listening on port %d...\n", MY_PORT);
    ...
    while (1) {
        ...
        conn_count++; // parent incr before fork so each child knows its connection num
        int go_to; // 0 for w26s, 1 for mirror1, 2 for mirror2
        if (conn_count <= 2) {
            go_to = 0;
            ...
            go_to = (conn_count - 7) % 3; // for conn beyond 6, cycle repeats
        }
        
        // log which server will handle this conn
        printf("Connection %d at %s\n", conn_count, go_to == 0 ? "w26server" : go_to == 1 ? "mirror1" : "mirror2");

        pid_t pid = fork(); // fork for client handling
        if (pid < 0) {
            perror("fork"); // log error if fails
            close(con_sd);
            continue;
        }

        if (pid == 0) { // child process to handle this client
            close(lis_sd);
            if (go_to == 0) {
                crequest(con_sd); // handle by this server
            } else if (go_to == 1) {
                send_to_mirror(con_sd, M1_PORT); // send to mirror1
            } else {
                send_to_mirror(con_sd, M2_PORT); // send to mirror2
            }
            exit(0);
        }
        close(con_sd); // parent closes the connected socket fd
    }
